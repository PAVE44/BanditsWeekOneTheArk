...
ZombieActions.Collect.onStart = function(zombie, task)
    ...
    task.surface = BWOAPrepareTools.GetSurfaceOffset(task.item.x, task.item.y, task.item.z)

    local anim
    if task.item.ground and task.surface < 0.2 then
        anim = "LootLow"
    else
        anim = "Loot"
    end
...
ZombieActions.Collect.onComplete = function(zombie, task)
    ...
    if task.item.ground then
        ...
        for i=0, wobs:size()-1 do
            ...
            if itemType == task.item.ftype and (task.cooked == nil or (task.cooked == item:isCooked())) then
        ...
        for i=0, objects:size() - 1 do
            ...
            if container then
                ...
                for i=0, items:size()-1 do
                    ...
                    if item:getFullType() == task.item.ftype and (task.cooked == nil or (task.cooked == item:isCooked())) then
                        BWOAPermaInv.Add(zombie, item)
