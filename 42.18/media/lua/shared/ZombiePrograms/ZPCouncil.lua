...
ZombiePrograms.Council = {}
ZombiePrograms.Council.Stages = {}

ZombiePrograms.Council.Init = function(bandit)
end

ZombiePrograms.Council.Prepare = function(bandit)
    local tasks = {}
...
ZombiePrograms.Council.Main = function(bandit)
    local tasks = {}
    ...
    if occupation then
        ...
        if occupation.texts and occupation.textColor then
            if BWOAScenes.Council.texts[occupation.texts] then
                texts = BWOAScenes.Council.texts[occupation.texts]
        ...
        elseif occupation.action == "Sit" then
            local task = {action="SitInChair", anim="SitInChair1", txt=text, txtColor=textColor, voice=voice, looped=true, fx = fx, fy = fy, time=200}
            table.insert(tasks, task)
        elseif occupation.action == "SitComputer" then
            ...
            table.insert(tasks, task)
        elseif occupation.action == "Patient" then
            local task = {action="Generic", anim="Patient", looped=true, fx = fx, fy = fy, time=200}
            table.insert(tasks, task)
