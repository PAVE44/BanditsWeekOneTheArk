...
BWOASequence.Salvation = function(params)
    local volume = getSoundManager():getSoundVolume()
    local player = getSpecificPlayer(0)
    local px, py, pz = player:getX(), player:getY(), player:getZ()
    local npcData, emma = BWOANPC.Get("Emma")

    BWOAEventControl.Add("FadeOut", {time=0, mute=false}, 0)
    BWOAEventControl.Add("PlayPlayer", {sound="Salvation"}, 1)
    -- BWOAEventControl.Add("PlayPlayer", {sound="DeathCrash"}, 2)
    BWOAEventControl.Add("Salvation", {}, 3)
    ...
    local h = 6000
    BWOAEventControl.Add("NPCTeleport", {name="Emma", x=px, y=py, z=pz}, h + 1)
    BWOAEventControl.Add("FadeIn", {time = 3, volume = volume, unmute=true}, h + 2200)
    BWOAEventControl.Add("Tex", tex, h + 2201)
    BWOAEventControl.Add("PlayPlayer", {sound="Dream9End"}, h + 2202)
    BWOAEventControl.Add("SayPlayer", {txt=getText("IGUI_SayPlayer_AmIAlive")}, h + 2900)

    -- BanditUtils.ClearZombies(px - 50, px + 50, py - 50, py + 50)
end
