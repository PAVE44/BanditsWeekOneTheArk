...
function UIMissions:initialise()
    ...
    for id, mission in pairs(missions) do
        ...
        self.missionListBox:addItem(id, tab)
    end
    ...
    self.missionListBox.doDrawItem = function(list, y, item, alt)
        ...
        if item.item.progressCnt then
            local cnt = 0
            if item.item.progress then
                for k, v in pairs(item.item.progress) do
                    if v then
                        cnt = cnt + 1
                    end
                end
            end
            local progress = math.floor((cnt / item.item.progressCnt) * 100)
            name = "[" .. progress .. "%] " .. name
        end

        local width1 = getTextManager():MeasureStringX(UIFont.Medium, name)
        local width2 = getTextManager():MeasureStringX(UIFont.Small, desc)
        ...
        list:drawText(name, 4, y + 6, alpha, alpha, alpha, 1, UIFont.Medium)
        list:drawText(desc, 4, y + 6 + FONT_HGT_MEDIUM, alpha, alpha, alpha, 1, UIFont.Small)
