...
local onPlayerUpdate = function(player)
    ...
    local obh = player:getBodyDamage():getOverallBodyHealth()
    local onFire = player:isOnFire()
    local gmd = GetBWOAModData()

    -- fire is too op
    if onFire then
        if ZombRand(50) == 0 and obh < 50 then
            player:StopBurning()
        end
    end

    -- salvation
    if not onFire and obh < 6 and not gmd.nightmares.active and gmd.salvation > 0 then
        BWOASequence.Salvation({})
        gmd.salvation = gmd.salvation - 1
    end
        ...
        for k_, placeEvent in pairs(placeEvents) do
            if not placeEvent.hidden then
                if math.abs(px - placeEvent.x) < 6 and math.abs(py - placeEvent.y) < 6 and pz == placeEvent.z then
                    ...
                    if placeEvent.revealMissionId then
                        BWOAMissions.Reveal(placeEvent.revealMissionId)
                    end
                    ...
                    if placeEvent.hideDialogueId and placeEvent.hideDialoguePerson then
                        BWOADialogues.Hide(placeEvent.hideDialoguePerson, placeEvent.hideDialogueId)
                    end
                    if placeEvent.progressMissionId then
                        BWOAMissions.Progress(placeEvent.progressMissionId, placeEvent.scene)
                    end
                end
            end
        end
    ...
    if instanceof(destContainerParent, "IsoPlayer") or instanceof(containerCharacter, "IsoPlayer") then 
        ...
        if md.BWOA and md.BWOA.onTaken then
            ...
            if onTaken.hideDialogueId and onTaken.hideDialoguePerson then
                BWOADialogues.Hide(onTaken.hideDialoguePerson, onTaken.hideDialogueId)
            end
            if onTaken.progressMissionId then
                BWOAMissions.Progress(onRead.progressMissionId, item:getFullType())
            end
        end
    ...
    -- this means dropping things or putting in container
        ...
        if md.BWOA and md.BWOA.onDropArea then
            ...
            if cx >= dropArea.x1 and cx <= dropArea.x2 and cy >= dropArea.y1 and cy <= dropArea.y2 and cz == dropArea.z then
                ...
                if dropArea.progressMissionId then
                    BWOAMissions.Progress(dropArea.progressMissionId, item:getFullType())
                end
            end
        end
    end
