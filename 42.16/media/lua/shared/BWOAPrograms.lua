...
BWOAPrograms.CleanBlood = function(bandit)
    ...
    local blood, dist = BWOABaseObjects.FindClosestBlood({x=bx, y=by}, 100)
    if blood then
...
BWOAPrograms.Cook = function(bandit, activityMode)
    local bx, by = bandit:getX(), bandit:getY()

    local recipe = BWOARecipes.GetRecipe("Oatmeal")
    if activityMode == "lunch" then
        recipe = BWOARecipes.GetRecipe("Salad")
    elseif activityMode == "dinner" then
        recipe = BWOARecipes.GetRecipe("Roast")
    end
    ...
    if not vesselReady then
        ...
        if not hasVessel then
            ...
            if item and not (math.floor(item.x) == makePosition.x and math.floor(item.y) == makePosition.y) then
                local task = {action="Collect", time=300, item=item}
                local subTasks = BWOAPrograms.GoAndDo(bandit, item, task)
                if #subTasks > 0 then return subTasks end
            end
